...
uniform vec3 SR_carpaint_base_color = vec3(0.051890, 0.296060, 0.425324);
...
uniform float SR_carpaint_alpha = 1.000000;
uniform int SR_carpaint_alpha_mode = 0;
uniform float SR_carpaint_alpha_cutoff = 0.500000;
uniform float SR_carpaint_iridescence = 0.000000;
uniform float SR_carpaint_iridescence_ior = 1.300000;
uniform float SR_carpaint_iridescence_thickness = 100.000000;
uniform vec3 SR_carpaint_sheen_color = vec3(0.000000, 0.000000, 0.000000);
uniform float SR_carpaint_sheen_roughness = 0.000000;
uniform float SR_carpaint_clearcoat = 1.000000;
uniform float SR_carpaint_clearcoat_roughness = 0.000000;
uniform vec3 SR_carpaint_emissive = vec3(0.000000, 0.000000, 0.000000);
uniform float SR_carpaint_emissive_strength = 1.000000;
uniform float SR_carpaint_thickness = 0.000000;
uniform float SR_carpaint_attenuation_distance = 0.0;
uniform vec3 SR_carpaint_attenuation_color = vec3(1.000000, 1.000000, 1.000000);
...
void IMPL_gltf_pbr_surfaceshader(vec3 base_color, float metallic, float roughness, vec3 normal, vec3 tangent, float occlusion, float transmission, float specular, vec3 specular_color, float ior, float alpha, int alpha_mode, float alpha_cutoff, float iridescence, float iridescence_ior, float iridescence_thickness, vec3 sheen_color, float sheen_roughness, float clearcoat, float clearcoat_roughness, vec3 clearcoat_normal, vec3 emissive, float emissive_strength, float thickness, float attenuation_distance, vec3 attenuation_color, out surfaceshader out1)
{
    ...
    surfaceshader shader_constructor_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Shadow occlusion
        vec3 shadowCoord = (u_shadowMatrix * vec4(vd.positionWorld, 1.0)).xyz;
        shadowCoord = shadowCoord * 0.5 + 0.5;
        vec2 shadowMoments = texture(u_shadowMap, shadowCoord.xy).xy;
...
void main()
{
    ...
    IMPL_gltf_pbr_surfaceshader(SR_carpaint_base_color, SR_carpaint_metallic, SR_carpaint_roughness, geomprop_Nworld_out1, geomprop_Tworld_out1, SR_carpaint_occlusion, SR_carpaint_transmission, SR_carpaint_specular, SR_carpaint_specular_color, SR_carpaint_ior, SR_carpaint_alpha, SR_carpaint_alpha_mode, SR_carpaint_alpha_cutoff, SR_carpaint_iridescence, SR_carpaint_iridescence_ior, SR_carpaint_iridescence_thickness, SR_carpaint_sheen_color, SR_carpaint_sheen_roughness, SR_carpaint_clearcoat, SR_carpaint_clearcoat_roughness, geomprop_Nworld_out1, SR_carpaint_emissive, SR_carpaint_emissive_strength, SR_carpaint_thickness, SR_carpaint_attenuation_distance, SR_carpaint_attenuation_color, SR_carpaint_out);
    material PBR_Car_Paint_out = SR_carpaint_out;
    float outAlpha = clamp(1.0 - dot(PBR_Car_Paint_out.transparency, vec3(0.3333)), 0.0, 1.0);
    out1 = vec4(PBR_Car_Paint_out.color, outAlpha);
